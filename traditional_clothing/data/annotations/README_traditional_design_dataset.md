...
|------|------|
| **有效样本** | **2,140 条** |
| 被形制规则过滤 | 722 条（如"袖口宽 = 袖根宽"违反 袖口 < 袖根 < 膨起） |
| **组件覆盖** | **12 个（全部）**：领型 4 · 袖型 3 · 下裳 2 · 配饰 3 |
| **朝代覆盖** | **6 代**：汉 330 / 魏晋 54 / 唐 432 / 宋 452 / 明 662 / 清 210 |
...
| 类别 | 组件 | 样本数 |
|------|------|--------|
| 领型 | 交领 cross-collar | 648 |
| 袖型 | 窄袖 narrow-sleeve | 300 |
| 下裳 | 马面裙 mamian-skirt | 216 |
| 领型 | 圆领 round-collar | 162 |
| 袖型 | 广袖 wide-sleeve | 162 |
| 下裳 | 襦裙 ruqun-skirt | 162 |
| 袖型 | 琵琶袖 pipa-sleeve | 144 |
| 领型 | 对襟 duijin-collar | 108 |
| 配饰 | 半臂 banbi | 96 |
| 领型 | 立领 stand-collar | 54 |
| 配饰 | 云肩 cloud-shoulder | 48 |
| 配饰 | 褙子 beizi | 40 |
...
2. **组件已全覆盖（12/12）**，但样本量分布不均（交领 648 vs 褙子 40），
   因为部分组件的形制约束更严（配饰的 warnings 过滤掉大量组合）。
3. **体型固定**：全部基于 `mean_female.yaml`（与论文一致："we use a standard body model throughout"）。
4. **配饰组件做了几何简化**（详见下）：
   - 云肩：每层整环云头片（背中开窄口），去掉独立领座
   - 褙子：前身单片倒 U 形 + 前中窄缝表达对襟（裁剪时沿线剪开即两襟）
   - 半臂：平裁连袖（前后各 1 片 T 形）
5. **形制约束来自项目规则**（23 条校验规则），非历史文献原文，建议领域专家复核。
6. **未做**：汉/魏晋/唐组件较少（广袖 162 / 襦裙 162），唐宋形制细节（如钿钗礼衣、
   圆领袍的襕）尚未建模。
